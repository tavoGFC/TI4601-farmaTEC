/*CONSULTAS PARA ADMINISTRADORES DE SAN JOS� Y CARTAGO*/

/*CANTIDAD DE DINERO RECAUDADO EN SUCURSAL*/
CREATE PROCEDURE sp_Get_Raised_Money
  @City VARCHAR(9)
AS
...
/*CANTIDAD DE PEDIDOS POR CLIENTE PARA RANGO DE FECHAS ESPEC�FICO*/
...
/*MONTO PROMEDIO PAGADO POR PEDIDO POR CLIENTE PARA RANGO DE FECHAS ESPEC�FICO*/
CREATE PROCEDURE sp_Get_Average_Payment
  @In_Date DATE,
  @Out_Date DATE
AS
...
CREATE PROCEDURE sp_Get_Month_Order_Type_Payment
  @Type VARCHAR(10),
  @Month VARCHAR(2)
AS
SELECT SUM(Total_Price)
...
/*MONTO RECAUDADO POR SUCURSAL POR PER�ODO*/
CREATE PROCEDURE sp_Get_Branch_Raised_Money
  @In_Date DATE,
  @Out_Date DATE
AS
...
/*MONTO RECAUDADO POR SUCURSAL Y TIPO POR PER�ODO*/
CREATE PROCEDURE sp_Get_Branch_Type_Raised_Money
  @In_Date DATE,
  @Out_Date DATE
AS
...
CREATE PROCEDURE sp_Get_Top_Three_Clients
  @In_Date DATE,
  @Out_Date DATE
AS
SELECT TOP 3
  Cl_ID, Cl_First_Name, Cl_Last_Name_1, Cl_Last_Name_2, SUM(Total_Price)
FROM Client INNER JOIN Order_Total ON Or_Client_ID = Cl_ID
WHERE Or_Date BETWEEN @In_Date AND @Out_Date
